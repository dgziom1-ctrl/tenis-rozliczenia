...
function FieldLabel({ children }) {
  return (
    <label style={{
      ...
      color: 'var(--cz-dim)', marginBottom: 8,
    }}>
      {children}
    </label>
  );
  ...
  return (
    <div style={{ background: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(6px)' }}
      ...
      <div style={{
        ...
        boxShadow: '0 0 60px rgba(232,89,10,0.15), 0 4px 60px rgba(0,0,0,0.95)',
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 22 }}>
          <div style={{
            ...
            <CheckCircle2 size={20} style={{ color: 'var(--cz-acid)' }} />
          </div>
          <div>
            <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.9rem', letterSpacing: '0.1em', color: 'var(--cz-acid)', marginBottom: 3, textTransform: 'uppercase' }}>
              Zapisano!
            </p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 16 }}>
          {[
            ...
            { label: 'KOSZT', value: `${formatAmountShort(totalCost)} ZŁ`, color: 'var(--cz-orange)' },
            { label: 'OBECNI', value: presentCount, color: '#c8c8c8' },
            multisportCount > 0
              ? { label: 'MULTISPORT', value: `⚡ ${multisportCount}`, color: 'var(--cz-acid)' }
              : { label: 'PŁACĄ', value: payingCount, color: '#c8c8c8' },
          ].map(({ label, value, color }) => (
            <div key={label} style={{
              padding: '10px 12px', background: 'var(--cz-dark)',
              border: '1px solid var(--cz-border)',
              clipPath: 'polygon(4px 0, 100% 0, calc(100% - 4px) 100%, 0 100%)',
            }}>
              <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.8rem', letterSpacing: '0.12em', color: 'var(--cz-dim)', marginBottom: 4, textTransform: 'uppercase' }}>{label}</p>
        ...
        {payingCount > 0 && (
          <div style={{
            ...
            border: '1px solid rgba(232,89,10,0.3)',
            clipPath: 'polygon(8px 0, 100% 0, calc(100% - 8px) 100%, 0 100%)',
          }}>
            <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.85rem', letterSpacing: '0.12em', color: 'var(--cz-dim)', marginBottom: 6, textTransform: 'uppercase' }}>
              KAŻDY PŁACI
            </p>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '2.4rem', color: 'var(--cz-orange)', textShadow: '0 0 20px rgba(232,89,10,0.5)', margin: 0 }}>
              {formatAmountShort(perPerson)}<span style={{ fontSize: '1rem', opacity: 0.4, marginLeft: 4 }}>ZŁ</span>
            </p>
            {multisportCount > 0 && (
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'var(--cz-dim)', marginTop: 4 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <button onClick={handleCopy} style={{
            ...
            ...(copied ? {
              background: 'rgba(127,255,0,0.08)', border: '1px solid rgba(127,255,0,0.4)', color: 'var(--cz-acid)',
            } : {
...
function LiveCostPreview({ totalCost, presentPlayers, multisportPlayers }) {
  ...
  return (
    <div style={{
      ...
      border: '1px solid rgba(232,89,10,0.2)',
      clipPath: 'polygon(6px 0, 100% 0, calc(100% - 6px) 100%, 0 100%)',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <Calculator size={14} style={{ color: 'var(--cz-dim)' }} />
        <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.88rem', letterSpacing: '0.15em', color: 'var(--cz-dim)', textTransform: 'uppercase' }}>
          Podział kosztów
        </span>
      </div>
      <div style={{ textAlign: 'right' }}>
        {paying.length > 0 ? (
          <>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: '1.4rem', color: 'var(--cz-orange)', textShadow: '0 0 10px rgba(232,89,10,0.3)' }}>
              {formatAmountShort(perPerson)} ZŁ
            </span>
            <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.85rem', color: 'var(--cz-dim)', marginLeft: 8, letterSpacing: '0.12em' }}>
              / os. ({paying.length} PŁACI)
            </span>
          </>
        ) : (
          <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.95rem', letterSpacing: '0.1em', color: 'var(--cz-acid)' }}>
...
function PlayerToggleGrid({ names, selected, onToggle, accent = 'yellow' }) {
  const accentColor = accent === 'green' ? 'var(--cz-acid)' : 'var(--cz-orange)';
  const accentAlpha = accent === 'green' ? 'rgba(127,255,0,0.08)' : 'rgba(232,89,10,0.08)';
  const accentBorder = accent === 'green' ? 'rgba(127,255,0,0.4)' : 'rgba(232,89,10,0.4)';
        ...
        return (
          <button type="button" key={name} onClick={() => onToggle(name)} style={{
            ...
            ...(active ? {
              ...
              boxShadow: `0 0 10px ${accent === 'green' ? 'rgba(127,255,0,0.1)' : 'rgba(232,89,10,0.1)'}`,
            } : {
              background: 'var(--cz-dark)', border: '1px solid var(--cz-border)', color: '#444',
            }),
          }}>
            {name}
          </button>
    ...
    <>
      ...
      <div style={{ width: '100%', maxWidth: 680, margin: '0 auto', animation: 'slide-in-up 0.3s ease-out' }}>
        <div className="cyber-box" style={{ clipPath: 'polygon(0 0, calc(100% - 20px) 0, 100% 20px, 100% 100%, 0 100%)', padding: '20px 20px' }}>
          ...
          {/* Header */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28, paddingBottom: 16, borderBottom: '1px solid var(--cz-border)' }}>
            <div style={{ padding: '7px 9px', background: 'rgba(232,89,10,0.07)', border: '1px solid rgba(232,89,10,0.25)', clipPath: 'polygon(4px 0, 100% 0, calc(100% - 4px) 100%, 0 100%)' }}>
              <CalendarPlus size={16} style={{ color: 'var(--cz-orange)', display: 'block' }} />
            </div>
            <div>
              <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.95rem', letterSpacing: '0.22em', color: 'var(--cz-orange)', textTransform: 'uppercase' }}>
                Dodaj nową sesję
              </span>
              <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'var(--cz-dim)', margin: '2px 0 0' }}>
          ...
          <form onSubmit={handleSaveSession} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                  ...
                  return (
                    <button type="button" key={cost}
                      ...
                      style={{
                        ...
                        clipPath: 'polygon(4px 0, 100% 0, calc(100% - 4px) 100%, 0 100%)',
                        ...(active ? {
                          background: 'rgba(232,89,10,0.1)', border: '1px solid rgba(232,89,10,0.5)',
                          color: 'var(--cz-orange)', boxShadow: '0 0 8px rgba(232,89,10,0.15)',
                        } : {
                          background: 'var(--cz-dark)', border: '1px solid var(--cz-border)', color: '#555',
                        }),
                      }}>
                      {cost === 0 ? 'FREE' : cost}
                    </button>
            ...
            {/* Present players */}
            <div style={{ padding: '16px', background: 'var(--cz-dark)', border: '1px solid var(--cz-border)', clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 0 100%)' }}>
              <p style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, fontFamily: 'var(--font-display)', fontSize: '0.9rem', letterSpacing: '0.18em', color: '#888', textTransform: 'uppercase' }}>
                <Users size={13} style={{ color: 'var(--cz-orange)' }} />
                Kto grał?
                <span style={{ marginLeft: 'auto', fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'var(--cz-orange)' }}>
            ...
            {presentPlayers.length > 0 && (
              <div style={{ padding: '16px', background: 'var(--cz-dark)', border: '1px solid var(--cz-border)', clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 0 100%)' }}>
                <p style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12, fontFamily: 'var(--font-display)', fontSize: '0.9rem', letterSpacing: '0.18em', color: '#888', textTransform: 'uppercase' }}>
                  <Zap size={13} style={{ color: 'var(--cz-acid)' }} />
                  Kto miał Multisport? (płaci 0 zł)
                  {multisportPlayers.length > 0 && (
                    <span style={{ marginLeft: 'auto', fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'var(--cz-acid)' }}>
            ...
            <div>
              <button type="submit" disabled={isDisabled}
                ...
                style={{
                  ...
                  ...(isDisabled ? {
                    background: 'var(--cz-panel)', border: '1px solid var(--cz-border)', color: '#333',
                  } : {}),
                }}>
                {isSaving
              ...
              {!isSaving && isDisabled && (
                <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.6rem', color: 'var(--cz-dim)', textAlign: 'center', marginTop: 8 }}>
