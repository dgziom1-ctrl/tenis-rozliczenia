...
const AVATAR_COLORS = [
  { bg: '#120800', border: 'var(--cz-orange)', text: 'var(--cz-orange)' },
  { bg: '#0a0010', border: '#a855f7', text: '#a855f7' },
  { bg: '#001000', border: 'var(--cz-acid)', text: 'var(--cz-acid)' },
  { bg: '#000a10', border: 'var(--cz-teal)', text: 'var(--cz-teal)' },
  { bg: '#100000', border: 'var(--cz-blood)', text: 'var(--cz-blood)' },
  { bg: '#080808', border: '#888', text: '#888' },
];

function SectionHeader({ icon: Icon, title, accent = 'var(--cz-orange)' }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, paddingBottom: 12, borderBottom: '1px solid var(--cz-border)' }}>
...
function PasswordModal({ playerName, onConfirm, onCancel, tokens }) {
  ...
  return (
    <div style={{ background: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(4px)' }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div style={{
        background: '#0a0a0f',
        border: `1px solid ${error ? 'var(--cz-blood)' : 'rgba(232,89,10,0.35)'}`,
        clipPath: 'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 16px 100%, 0 calc(100% - 16px))',
        padding: 24, width: '100%', maxWidth: 360,
        boxShadow: error ? '0 0 30px rgba(255,0,51,0.25)' : '0 0 30px rgba(232,89,10,0.12)',
        transition: 'all 0.2s',
      }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          />
          {error && <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.85rem', letterSpacing: '0.15em', color: 'var(--cz-blood)', textAlign: 'center' }}>⚠ ❌ Złe hasło</p>}
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {isOrganizer
          ? <Shield size={16} style={{ color: 'var(--cz-teal)' }} />
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.82rem', letterSpacing: '0.15em', color: isOrganizer ? 'var(--cz-teal)' : c.border, opacity: 0.7, margin: '2px 0 0', textTransform: 'uppercase' }}>
      ...
      {!isOrganizer ? (
        <button onClick={() => onDelete(player.name)} style={{
          ...
          onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(255,0,51,0.5)'; e.currentTarget.style.color = 'var(--cz-blood)'; }}
        ...
        <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.82rem', letterSpacing: '0.12em', color: 'var(--cz-dim)', padding: '4px 8px', border: '1px solid var(--cz-border)', clipPath: 'polygon(4px 0, 100% 0, calc(100% - 4px) 100%, 0 100%)' }}>
          HQ
        </span>
      )}
    </div>
    ...
    <>
      ...
      <div className="cyber-box" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28, paddingBottom: 16, borderBottom: '1px solid var(--cz-border)' }}>
          <div style={{ padding: '7px 9px', background: 'rgba(232,89,10,0.06)', border: '1px solid rgba(232,89,10,0.2)', clipPath: 'polygon(4px 0, 100% 0, calc(100% - 4px) 100%, 0 100%)' }}>
            <Users size={16} style={{ color: 'var(--cz-orange)', display: 'block' }} />
          </div>
          <div>
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.95rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--cz-orange)' }}>
              Gracze
            </span>
        ...
        {players && players.length > 0 && (
          <div style={{ marginBottom: 28 }}>
            <SectionHeader icon={Zap} title="Multisport domyślny" accent="var(--cz-acid)" />
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--cz-dim)', marginBottom: 12 }}>
                ...
                return (
                  <button key={p.name} onClick={() => toggleDefaultMulti(p.name)} disabled={savingMulti}
                    style={{
                      ...
                      ...(active ? {
                        background: 'rgba(127,255,0,0.07)', border: '1px solid rgba(127,255,0,0.4)', color: 'var(--cz-acid)',
                        boxShadow: '0 0 8px rgba(127,255,0,0.1)',
                      } : {
                        background: 'var(--cz-dark)', border: '1px solid var(--cz-border)', color: '#444',
                      }),
                    }}>
                    <Zap size={12} style={{ color: active ? 'var(--cz-acid)' : '#333', flexShrink: 0 }} />
                    {p.name}
                  </button>
                );
              })}
        ...
        {deletedPlayers?.length > 0 && (
          <div>
            <SectionHeader icon={Trash2} title="Kosz" accent="var(--cz-blood)" />
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 8 }}>
                ...
                <div key={name}>
                  {confirmDelete === name ? (
                    <div style={{
                      ...
                      <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.9rem', letterSpacing: '0.12em', color: 'var(--cz-blood)', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 5, textTransform: 'uppercase' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button onClick={() => handlePermanentDelete(name)} style={{
                          flex: 1, padding: '8px', background: 'var(--cz-blood)', color: '#000', border: 'none', cursor: 'pointer',
                    ...
                    <div style={{
                      ...
                      padding: '10px 12px', background: 'var(--cz-dark)', border: '1px solid var(--cz-border)',
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button onClick={() => handleRestore(name)} style={{
                          padding: '6px 8px', background: 'transparent', border: '1px solid var(--cz-border)', cursor: 'pointer',
                          color: '#3a3a3a', transition: 'all 0.15s',
                          clipPath: 'polygon(3px 0, 100% 0, calc(100% - 3px) 100%, 0 100%)',
                        }}
                          onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(127,255,0,0.4)'; e.currentTarget.style.color = 'var(--cz-acid)'; }}
                        ...
                        <button onClick={() => setConfirmDelete(name)} style={{
                          ...
                          onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(255,0,51,0.4)'; e.currentTarget.style.color = 'var(--cz-blood)'; }}
                          onMouseLeave={e => { e.currentTarget.style.borderColor = '#1e1e1e'; e.currentTarget.style.color = '#3a3a3a'; }}
