  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <button onClick={onToggle} style={{
        ...
        letterSpacing: '0.14em', color: 'rgba(255,0,255,0.5)', textTransform: 'uppercase',
      }}
        onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(255,0,255,0.25)'; e.currentTarget.style.color = 'var(--sw-pink)'; }}
        onMouseLeave={e => { e.currentTarget.style.borderColor = '#1a1a1a'; e.currentTarget.style.color = 'rgba(255,0,255,0.5)'; }}
      >
        ...
        <div style={{
          ...
          {breakdown.sessions.length > 0 ? (
            <>
              ...
              {breakdown.sessions.map((item, idx) => (
                <TerminalRow key={idx}>
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: 'var(--sw-pending)', fontWeight: 600 }}>
                    -{formatAmountShort(item.amount)} ZŁ
                  </span>
                </TerminalRow>
              ))}
              {breakdown.sessions.length > 1 && (
                <TerminalRow highlight="rgba(139,130,204,0.04)">
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: 'var(--sw-pending)' }}>
            ...
            <TerminalRow>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: 'var(--sw-settled)' }}>✓</span>
            </TerminalRow>
          )}
          ...
          {breakdown.payments.length > 0 && (
            <>
              ...
              {breakdown.payments.map((item, idx) => (
                <TerminalRow key={idx}>
                  ...
                  <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: 'var(--sw-pink)', fontWeight: 600 }}>
                      +{formatAmountShort(item.amount)} ZŁ
                    </span>
                    {adminMode && item.id !== '__legacy_settled__' && (
                      <button onClick={() => onRemovePayment(playerName, item.id)} style={{
                        background: 'transparent', border: 'none', cursor: 'pointer',
                        color: 'var(--sw-pending)', opacity: 0.4, padding: '1px 2px', lineHeight: 1,
              ...
              {breakdown.payments.length > 1 && (
                <TerminalRow highlight="rgba(255,0,255,0.03)">
                  ...
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: 'var(--sw-pink)' }}>
          ...
          <div style={{
            ...
            <span style={{
              ...
              color: breakdown.balance > 0.01 ? 'var(--sw-pending)'
                   : breakdown.balance < -0.01 ? 'var(--sw-pink)'
                   : 'var(--sw-settled)',
            }}>
...
function TerminalSectionHeader({ label }) {
  return (
    <div style={{
      ...
      <span style={{ color: 'var(--sw-settled)', fontSize: '0.66rem' }}>{'>'}</span>
      {label}
    </div>
  );
}
