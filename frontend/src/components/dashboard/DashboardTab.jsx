...
function SystemStatusBar({ debtCount, settledCount, totalWeeks }) {
  ...
  return (
    <div style={{
      ...
      borderLeft: `3px solid ${allClear ? 'var(--cz-acid)' : debtCount > 0 ? 'var(--cz-blood)' : 'var(--cz-orange)'}`,
      clipPath: 'polygon(0 0, calc(100% - 10px) 0, 100% 10px, 100% 100%, 10px 100%, 0 calc(100% - 10px))',
      flexWrap: 'wrap', gap: 8,
    }}>
      {/* Title */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        <Database size={12} style={{ color: 'var(--cz-orange)', flexShrink: 0 }} />
        <span style={{
      ...
      <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <div style={{
            width: 6, height: 6, background: 'var(--cz-blood)',
            boxShadow: '0 0 5px var(--cz-blood)',
            clipPath: 'polygon(50% 0, 100% 100%, 0 100%)',
          }} />
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: '#AAA' }}>
            Zaległości: <span style={{ color: debtCount > 0 ? 'var(--cz-blood)' : '#AAA' }}>{debtCount}</span>
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <div style={{
            width: 6, height: 6, borderRadius: '50%',
            background: 'var(--cz-acid)', boxShadow: '0 0 5px var(--cz-acid)',
          }} />
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: '#AAA' }}>
            Rozliczeni: <span style={{ color: 'var(--cz-acid)' }}>{settledCount}</span>
          </span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <Zap size={10} style={{ color: 'var(--cz-orange)' }} />
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: '0.7rem', color: '#AAA' }}>
            Sesje: <span style={{ color: 'var(--cz-orange)' }}>{totalWeeks}</span>
          </span>
        </div>
      </div>
    </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {totalWeeks === 0 && (
          <div style={{
            ...
            <p style={{ fontFamily: 'var(--font-display)', fontSize: '1.2rem', letterSpacing: '0.1em', color: 'var(--cz-orange)', marginBottom: 8 }}>
              BRAK ROZGRYWEK
            </p>
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.75rem', color: 'var(--cz-dim)' }}>
              {'>'} Dodaj pierwszą sesję w zakładce{' '}
              <span style={{ color: 'var(--cz-orange)' }}>LOG</span>
            </p>
          </div>
        )}
