...
/** Punkt po rzucie perspektywicznym: [x ekranu, y ekranu, skala]. */
...
export default function ArenaCanvas({ chaosMode, onHit }: ArenaCanvasProps) {
  ...
  useEffect(() => {
    ...
    if (sg) {
      ...
      const quadS = (pts: ScreenPoint[], fill: string | CanvasGradient) => {
        sg.beginPath();
      ...
      const segS = ([ax, ay]: ScreenPoint, [bx, by]: ScreenPoint, col: string, w: number) => {
        sg.beginPath();
        sg.moveTo(ax, ay);
        sg.lineTo(bx, by);
        sg.strokeStyle = col;
