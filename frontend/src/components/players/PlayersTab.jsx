...
const AVATAR_COLORS = [
  { bg: '#120800', border: 'var(--sw-pink)', text: 'var(--sw-pink)' },
  { bg: '#0a0010', border: '#a855f7', text: '#a855f7' },
  { bg: '#001000', border: 'var(--sw-settled)', text: 'var(--sw-settled)' },
  { bg: '#000a10', border: 'var(--sw-cyan)', text: 'var(--sw-cyan)' },
  { bg: '#100000', border: 'var(--sw-pending)', text: 'var(--sw-pending)' },
  { bg: '#080808', border: '#888', text: '#888' },
];

function SectionHeader({ icon: Icon, title, accent = 'var(--sw-pink)' }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16, paddingBottom: 12, borderBottom: '1px solid var(--sw-border)' }}>
...
function PasswordModal({ playerName, onConfirm, onCancel, tokens }) {
  ...
  return (
    <div style={{ background: 'rgba(0,0,0,0.95)', backdropFilter: 'blur(4px)' }} className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div style={{
        background: '#0a0a0f',
        border: `1px solid ${error ? 'var(--sw-pending)' : 'rgba(255,0,255,0.35)'}`,
        clipPath: 'polygon(0 0, calc(100% - 16px) 0, 100% 16px, 100% 100%, 16px 100%, 0 calc(100% - 16px))',
        padding: 24, width: '100%', maxWidth: 360,
        boxShadow: error ? '0 0 30px rgba(139,130,204,0.25)' : '0 0 30px rgba(255,0,255,0.12)',
        transition: 'all 0.2s',
      }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          <input type="password" value={input} onChange={e => setInput(e.target.value)}
            ...
            style={{
              ...
              boxShadow: error ? '0 0 10px rgba(139,130,204,0.3)' : 'none',
            }}
          />
          {error && <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.85rem', letterSpacing: '0.15em', color: 'var(--sw-pending)', textAlign: 'center' }}>⚠ ❌ Złe hasło</p>}
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {isOrganizer
          ? <Shield size={16} style={{ color: 'var(--sw-cyan)' }} />
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.82rem', letterSpacing: '0.15em', color: isOrganizer ? 'var(--sw-cyan)' : c.border, opacity: 0.7, margin: '2px 0 0', textTransform: 'uppercase' }}>
      ...
      {!isOrganizer ? (
        ...
        <span style={{ fontFamily: 'var(--font-display)', fontSize: '0.82rem', letterSpacing: '0.12em', color: 'var(--sw-dim)', padding: '4px 8px', border: '1px solid var(--sw-border)', clipPath: 'polygon(4px 0, 100% 0, calc(100% - 4px) 100%, 0 100%)' }}>
          HQ
        </span>
      )}
    </div>
    ...
    <>
      ...
      <div className="cyber-box" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 28, paddingBottom: 16, borderBottom: '1px solid var(--sw-border)' }}>
          <div style={{ padding: '7px 9px', background: 'rgba(255,0,255,0.06)', border: '1px solid rgba(255,0,255,0.2)', clipPath: 'polygon(4px 0, 100% 0, calc(100% - 4px) 100%, 0 100%)' }}>
            <Users size={16} style={{ color: 'var(--sw-pink)', display: 'block' }} />
          </div>
          <div>
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.95rem', letterSpacing: '0.22em', textTransform: 'uppercase', color: 'var(--sw-pink)' }}>
              Gracze
            </span>
        ...
        {players && players.length > 0 && (
          <div style={{ marginBottom: 28 }}>
            <SectionHeader icon={Zap} title="Multisport domyślny" accent="var(--sw-settled)" />
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '0.65rem', color: 'var(--sw-dim)', marginBottom: 12 }}>
                ...
                return (
                  <button key={p.name} onClick={() => toggleDefaultMulti(p.name)} disabled={savingMulti}
                    style={{
                      ...
                      ...(active ? {
                        background: 'rgba(0,255,255,0.07)', border: '1px solid rgba(0,255,255,0.4)', color: 'var(--sw-settled)',
                        boxShadow: '0 0 8px rgba(0,255,255,0.1)',
                      } : {
                        background: 'var(--sw-dark)', border: '1px solid var(--sw-border)', color: '#444',
                      }),
                    }}>
                    <Zap size={12} style={{ color: active ? 'var(--sw-settled)' : '#333', flexShrink: 0 }} />
                    {p.name}
                  </button>
                );
              })}
        ...
        {deletedPlayers?.length > 0 && (
          <div>
            <SectionHeader icon={Trash2} title="Kosz" accent="var(--sw-pending)" />
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 8 }}>
              {deletedPlayers.map(name => (
                <div key={name}>
                  {confirmDelete === name ? (
                    <div style={{
                      padding: '14px', background: 'rgba(139,130,204,0.04)', border: '1px solid rgba(139,130,204,0.3)',
                      clipPath: 'polygon(0 0, calc(100% - 10px) 0, 100% 10px, 100% 100%, 0 100%)',
                    }}>
                      <p style={{ fontFamily: 'var(--font-display)', fontSize: '0.9rem', letterSpacing: '0.12em', color: 'var(--sw-pending)', marginBottom: 4, display: 'flex', alignItems: 'center', gap: 5, textTransform: 'uppercase' }}>
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button onClick={() => handlePermanentDelete(name)} style={{
                          flex: 1, padding: '8px', background: 'var(--sw-pending)', color: '#000', border: 'none', cursor: 'pointer',
                    ...
                    <div style={{
                      ...
                      padding: '10px 12px', background: 'var(--sw-dark)', border: '1px solid var(--sw-border)',
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button onClick={() => handleRestore(name)} style={{
                          padding: '6px 8px', background: 'transparent', border: '1px solid var(--sw-border)', cursor: 'pointer',
                          color: '#3a3a3a', transition: 'all 0.15s',
                          clipPath: 'polygon(3px 0, 100% 0, calc(100% - 3px) 100%, 0 100%)',
                        }}
                          onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(0,255,255,0.4)'; e.currentTarget.style.color = 'var(--sw-settled)'; }}
                        ...
                        <button onClick={() => setConfirmDelete(name)} style={{
                          ...
                          onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(139,130,204,0.4)'; e.currentTarget.style.color = 'var(--sw-pending)'; }}
                          onMouseLeave={e => { e.currentTarget.style.borderColor = '#1e1e1e'; e.currentTarget.style.color = '#3a3a3a'; }}
