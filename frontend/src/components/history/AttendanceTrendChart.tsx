...
export default function AttendanceTrendChart({ history }: AttendanceTrendChartProps) {
  const W = 560, H = 140, PAD = { top: 18, right: 46, bottom: 32, left: 36 };
      ...
      <div style={{ position: 'relative', background: 'rgba(0,229,255,0.015)', border: '1px solid var(--co-border)', clipPath: 'polygon(0 0, calc(100% - 12px) 0, 100% 12px, 100% 100%, 0 100%)', overflow: 'hidden' }}>
        <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="Wykres trendu frekwencji" style={{ display: 'block', width: '100%', height: 'auto' }}>
          ...
          {gridLines.map(({ y, label, costLabel }) => (
            <g key={y}>
              ...
              <text x={W - PAD.right + 6} y={y + 4} textAnchor="start"
                fill="rgba(255,32,144,0.4)" fontSize="9" fontFamily="Space Mono, monospace">
                {costLabel}
              </text>
            </g>
          ))}
          ...
          <text x={PAD.left - 6} y={PAD.top - 6} textAnchor="end"
            fill="rgba(0,229,255,0.5)" fontSize="8" fontFamily="Space Mono, monospace">
            os.
          </text>
          <text x={W - PAD.right + 6} y={PAD.top - 6} textAnchor="start"
            fill="rgba(255,32,144,0.55)" fontSize="8" fontFamily="Space Mono, monospace">
            zł
          </text>
