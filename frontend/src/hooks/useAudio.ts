...
/** Safari do dziś wystawia AudioContext wyłącznie pod prefiksem webkit. */
...
export function useAudio(isMuted: boolean) {
  ...
  // Przeglądarki limitują liczbę żywych AudioContextów, więc zamykamy własny
  // przy odmontowaniu — inaczej dźwięki cichną po kilku przeładowaniach widoku.
  useEffect(() => () => {
    audioContextRef.current?.close().catch(() => {});
    audioContextRef.current = null;
  }, []);

  const playCyber = useCallback((ctx: AudioContext, type: SoundType) => {
    if (!ctx) return;
    try {
  ...
  const vibrate = useCallback((type: SoundType) => {
