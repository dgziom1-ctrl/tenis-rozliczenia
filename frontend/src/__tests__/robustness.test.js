...
import { setCurrentData } from '../firebase/state';

// ─── vi.hoisted: runs BEFORE vi.mock factories, so mockSaveData is defined ───
const mockSaveData = vi.hoisted(() => vi.fn().mockResolvedValue(undefined));
...
vi.mock('../firebase/subscriptions', async (importOriginal) => {
  const actual = await importOriginal();
  return { ...actual, saveData: mockSaveData };
});
...
// ══════════════════════════════════════════════════════════════════════════════
// calculateDebtBreakdown — direction & edge cases
// ══════════════════════════════════════════════════════════════════════════════
...
describe('buildUIData — integration', () => {
  ...
  it('organizer not counted in totalToCollect', () => {
    // Kamil + Alice, cost 60. Both present, no multisport.
    // payingPlayers = ['Kamil','Alice'] → each pays 30.
    // Kamil is organizer → excluded from summary.
    // totalToCollect = Alice's debt only = 30.
    const raw = {
      players: ['Kamil', 'Alice'],
    ...
    expect(r.players.find(p => p.name === 'Kamil')).toBeUndefined();
  });
...
// ══════════════════════════════════════════════════════════════════════════════
// deleteWeek — paidUntilWeek cleanup
// ══════════════════════════════════════════════════════════════════════════════
...
describe('deleteWeek — paidUntilWeek cleanup', () => {
  ...
  beforeEach(() => {
    savedData = null;
    mockSaveData.mockImplementation(async (d) => {
  ...
  it('only updates affected player', async () => {
    ...
    await deleteWeek('w2');
    expect(savedData.paidUntilWeek.Alice).toBe('w1');
    expect(savedData.paidUntilWeek.Bob).toBe('w3');
  });
  ...
  it('removes all markers when only week deleted', async () => {
    ...
    await deleteWeek('w1');
    expect(savedData.paidUntilWeek.Alice).toBeUndefined();
    expect(savedData.paidUntilWeek.Bob).toBeUndefined();
  });
  ...
  it('returns error when not connected', async () => {
    setCurrentData(null);
    expect((await deleteWeek('w1')).success).toBe(false);
  });
});
...
describe('addSession — validation', () => {
  ...
  it('returns error when not connected', async () => {
    setCurrentData(null);
    expect((await addSession({ datePlayed: '2025-01-01', totalCost: 60, presentPlayers: ['Alice'] })).success).toBe(false);
  });
});
...
describe('addPlayer — robustness', () => {
  ...
  it('sets playerJoinWeek to current week count', async () => {
    let captured;
    mockSaveData.mockImplementation(async (d) => { captured = JSON.parse(JSON.stringify(d)); });
    seed({ players: ['Alice'], weeks: [{ id: 'w1' }, { id: 'w2' }] });
    await addPlayer('NewGuy');
    expect(captured.playerJoinWeek.NewGuy).toBe(2);
  });

  it('returns error when not connected', async () => {
    setCurrentData(null);
    expect((await addPlayer('Bob')).success).toBe(false);
  });
});
...
describe('settlePlayer / undoSettle — state machine', () => {
  ...
  beforeEach(() => {
    savedData = null;
    mockSaveData.mockImplementation(async (d) => { savedData = JSON.parse(JSON.stringify(d)); });
  });

  it('settles to last week ID', async () => {
    seed({ weeks: [{ id: 'w1' }, { id: 'w2' }, { id: 'w3' }], paidUntilWeek: {} });
    await settlePlayer('Alice');
    expect(savedData.paidUntilWeek.Alice).toBe('w3');
  });
  ...
  it('settle → undo → original debt round-trip', async () => {
    ...
    expect(calculateDebt('Alice', { weeks, paidUntilWeek: savedData.paidUntilWeek })).toBe(0);
    await undoSettle('Alice', previousValue);
    expect(calculateDebt('Alice', { weeks, paidUntilWeek: savedData.paidUntilWeek })).toBe(debtBefore);
  });
});
...
describe('softDeletePlayer / restorePlayer', () => {
  ...
  beforeEach(() => {
    savedData = null;
    mockSaveData.mockImplementation(async (d) => { savedData = JSON.parse(JSON.stringify(d)); });
    seed({ players: ['Alice', 'Bob'], deletedPlayers: [], weeks: [] });
  });

  it('moves player from active to deleted', async () => {
    await softDeletePlayer('Alice');
    expect(savedData.players).not.toContain('Alice');
    expect(savedData.deletedPlayers).toContain('Alice');
  });

  it('restores player back to active', async () => {
    seed({ players: ['Bob'], deletedPlayers: ['Alice'], weeks: [] });
    await restorePlayer('Alice');
    expect(savedData.players).toContain('Alice');
    expect(savedData.deletedPlayers).not.toContain('Alice');
  });

  it('deleting non-existent player is a no-op', async () => {
    await softDeletePlayer('Ghost');
    expect(savedData.players).toContain('Alice');
    expect(savedData.players).toContain('Bob');
  });
});
...
describe('updateWeek — edge cases', () => {
  ...
  it('updates only target week', async () => {
    ...
    expect(savedData.weeks[0].cost).toBe(90);
    expect(savedData.weeks[1].cost).toBe(60);
  });

  it('returns error when not connected', async () => {
    setCurrentData(null);
    expect((await updateWeek('w1', { date: '2025-01-01', cost: 60, present: ['Alice'], multiPlayers: [] })).success).toBe(false);
  });
});
